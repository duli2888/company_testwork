...
{
	public:
		CThreadQueue(int queueSize=1024):sizeQueue(queueSize),lput(0),lget(0),nFullThread(0),nEmptyThread(0),nData(0) 
		{
		...
		void *getq()
		{
			 ...
			 * 另一个线程醒来的时候已经没有新数据可以消耗了。另一点，man pthread_cond_wait可以看到，该函数可以被信号中断返回，
			 * 此时返回EINTR。为避免以上任何一点，都必须醒来后再次判断睡眠条件。更正：pthread_cond_wait是信号安全的系统调用，不会被信号中断。
			 */
