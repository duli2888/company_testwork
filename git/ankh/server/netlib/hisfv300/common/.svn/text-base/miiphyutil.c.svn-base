				    ...
				    unsigned char reg, unsigned short value))
{
    ...
    debug("enter miiphy_register(name='%s')\n",name);// 2012-04-18 sunray
...
int miiphy_set_current_dev (char *devname)
{
	...
	list_for_each (entry, &mii_devs) {
		...
		if (strcmp (devname, dev->name) == 0) {
        	printf ("miiphy_set_current_dev('%s') OK!\n", devname);// 2012-04-18 sunray
			current_mii = dev;
			return 0;
		}
	}
		 ...
		 unsigned short *value)
{
    ...
    //debug("miiphy_read(devname='%s',addr=0x%x,reg=0x%x) start!\n",devname,addr,reg);// 2012-04-18 sunray
	if (!devname) {
	...
	list_for_each (entry, &mii_devs) {
		...
		if (strcmp (devname, dev->name) == 0) {
            ...
            //debug("miiphy_read(devname='%s',addr=0x%x,reg=0x%x,value=0x%x) return %d!\n",
            //             devname,addr,reg,*value,read_ret);// 2012-04-18 sunray
			break;
		}
	}
		  ...
		  unsigned short value)
{
    ...
    //debug("miiphy_write(devname='%s',addr=0x%x,reg=0x%x,value=0x%x) start!\n",
    //             devname,addr,reg,value);// 2012-04-18 sunray
	if (!devname) {
		printf ("NULL device name!\n");
		return 1;
	}
	...
	list_for_each (entry, &mii_devs) {
		...
		if (strcmp (devname, dev->name) == 0) {
            ...
            //debug("miiphy_write(devname='%s',addr=0x%x,reg=0x%x,value=0x%x) return %d!\n",
            //     devname,addr,reg,value,write_ret);// 2012-04-18 sunray
			break;
		}
	}
...
int miiphy_reset (char *devname, unsigned char addr)
{
    ...
    //debug("miiphy_reset(devname='%s',addr=0x%x)\n",devname,addr);// 2012-04-18 sunray
	...
	if ((reg & 0x8000) == 0) {
        debug("miiphy_reset(devname='%s',addr=0x%x) OK\n",devname,addr);// 2012-04-18 sunray
		return (0);
	} else {
...
int miiphy_duplex (char *devname, unsigned char addr)
{
    ...
    // 2012-04-12,sunray, code does not go here
    debug("val & BMSR_ESTATEN == 1\n");

	/* Check for 1000BASE-X. */
	if (miiphy_is_1000base_x (devname, addr)) {
		/* 1000BASE-X */
        debug("miiphy_is_1000base_x() return 1\n");// 2012-04-18 sunray
		if (miiphy_read (devname, addr, PHY_ANLPAR, &anlpar)) {
    ...
    // 2012-04-12,sunray, code goes here
    //debug("val & BMSR_ESTATEN == 0\n");
	/* Check Basic Management Control Register first. */
	if (miiphy_read (devname, addr, PHY_BMCR, &bmcr)) {
		puts ("PHY duplex");
		goto miiphy_read_failed;
	...
	/* Get speed from basic control settings. */
    debug("bmcr & PHY_BMCR_DPLX == %d\n",bmcr & PHY_BMCR_DPLX);// 2012-04-18 sunray
...
int miiphy_link (char *devname, unsigned char addr)
{
    ...
    //printf("miiphy_link(devname='%s',addr=0x%x) %s!\n",
    //          devname,addr, (reg & PHY_BMSR_LS) != 0 ? "active":"inactive");// 2012-04-18 sunray
